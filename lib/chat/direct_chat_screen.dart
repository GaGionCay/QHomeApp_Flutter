...
//         (_viewModel.error!.contains('hidden') ||
//             _viewModel.error!.contains('Hidden'))) {
...
//             const SnackBar(
//               content: Text(
//                   'Cuộc trò chuyện đã bị xóa. Tin nhắn mới sẽ xuất hiện lại khi có tin nhắn mới.'),
...
//     print(
//         '📢 [DirectChatScreen] dispose() called - emitting direct_chat_activity_updated event');
...
//           _viewModel.conversation?.isBlockedByOther == true;
...
//     print(
//         '   Content preview: ${content.substring(0, content.length > 50 ? 50 : content.length)}...');
...
//         print(
//             '✅ [DirectChatScreen] Conversation loaded. Status: ${loadedConversation?.status}');
...
//       print(
//           '⚠️ [DirectChatScreen] Conversation is not ACTIVE: ${finalConversation.status}');
...
//         print(
//             '✅ [DirectChatScreen] Conversation refreshed. New status: ${refreshedConversation?.status}');
...
//           print(
//               '⚠️ [DirectChatScreen] Conversation status is still not ACTIVE: ${updatedConversation.status}');
...
//           print(
//               '⚠️ [DirectChatScreen] Allowing message send despite non-ACTIVE status: ${updatedConversation.status}');
//         }
//       } catch (e, stackTrace) {
...
//     try {
//       print(
//           '🎥 [DirectChatScreen] Bắt đầu chọn video từ ${source == ImageSource.gallery ? "gallery" : "camera"}');
...
//       print(
//           '📹 [DirectChatScreen] Video gốc duration: ${originalDuration}ms (${(originalDuration / 1000).toStringAsFixed(2)}s)');
...
//             const SnackBar(
//               content: Text(
//                   '❌ Video có thời lượng quá dài. Vui lòng chọn video ngắn hơn 10 giây.'),
//               backgroundColor: Colors.red,
...
//           print(
//               '✅ [DirectChatScreen] Video đã được compress: ${compressedVideo.path}');

//           // Double-check duration after compression (with buffer)
//           final compressedMediaInfo =
//               await VideoCompress.getMediaInfo(compressedVideo.path!);
//           final compressedDuration = compressedMediaInfo.duration ?? 0;
//           print(
//               '📹 [DirectChatScreen] Video sau compress duration: ${compressedDuration}ms (${(compressedDuration / 1000).toStringAsFixed(2)}s)');
...
//                 const SnackBar(
//                   content: Text(
//                       '❌ Video có thời lượng quá dài sau khi xử lý. Vui lòng chọn video ngắn hơn 10 giây.'),
//                   backgroundColor: Colors.red,
//                   duration: Duration(seconds: 4),
//                 ),
//               );
...
//             final savedPath =
//                 await PublicFileStorageService.saveToPublicDirectory(
//               finalVideoFile,
//               fileName,
//               'video',
//               'video/mp4',
//             );
//             print(
//                 '✅ [DirectChatScreen] Video đã được lưu vào public storage: $savedPath');
//           } catch (e) {
//             print(
//                 '⚠️ [DirectChatScreen] Lỗi khi lưu video vào public storage: $e');
...
//         const RecordConfig(
//           encoder: AudioEncoder.aacLc, // tương đương aacMP4
//           bitRate: 128000,
//           sampleRate: 44100,
//         ),
//         path: path,
...
//               const SnackBar(
//                 content:
//                     Text('Không thể gửi ghi âm: Cuộc trò chuyện đã bị chặn'),
//                 backgroundColor: Colors.red,
//               ),
//             );
//           }
...
//           );
//           print(
//               '✅ [DirectChatScreen] Audio file saved to public storage: $savedPath');
...
//         } catch (e) {
//           print(
//               '⚠️ [DirectChatScreen] Failed to save audio to public storage: $e');
...
//               lastMessage.id,
//               file.path,
//               PublicFileStorageService.getFileType(
//                   result.files.single.extension, fileName),
...
//       BuildContext context, DirectMessage message) async {
//     if (message.imageUrl == null) return;

//     // Only allow delete for messages sent by current user
//     final isMyMessage =
//         _currentResidentId != null && message.senderId == _currentResidentId;
...
//               ListTile(
//                 leading:
//                     const Icon(CupertinoIcons.delete_simple, color: Colors.red),
//                 title: const Text('Xóa ở phía mọi người',
//                     style: TextStyle(color: Colors.red)),
...
//   Future<void> _showVideoOptionsBottomSheet(
//       BuildContext context, DirectMessage message) async {
...
//       final mimeType =
//           PublicFileStorageService.getMimeTypeFromFileName(fileName);
...
//   Future<void> _showMessageOptionsBottomSheet(
//       BuildContext context, DirectMessage message) async {
...
//       BuildContext context, DirectMessage message, String deleteType) async {
...
//             _viewModel.conversation!.getOtherParticipantId(_currentResidentId!);
...
//         content:
//             const Text('Bạn có chắc chắn muốn gỡ chặn người dùng này không?'),
//         actions: [
//           TextButton(
//             onPressed: () => Navigator.pop(context, false),
//             child: const Text('Hủy'),
...
//           const SnackBar(
//             content: Text(
//                 'Đã bỏ chặn người dùng. Tin nhắn đã gửi trong lúc chặn sẽ được hiển thị.'),
//             duration: Duration(seconds: 3),
//           ),
//         );
//       }
...
//                   itemBuilder: (context) {
//                     final isBlocked =
//                         _viewModel.conversation?.status == 'BLOCKED';
//                     return [
//                       const PopupMenuItem(
//                         value: 'files',
//                         child: Row(
...
//                         'direct_chat_messages_list'),
//                     itemCount:
//                         viewModel.messages.length + (viewModel.hasMore ? 1 : 0),
...
//                       final messageKey =
//                           ValueKey<String>('message_${message.id}');
...
//                     isBlocked ? _getBlockedPlaceholderText() : null;
...
//           final errorMessage =
//               (e.message?.toLowerCase() ?? '') + (e.toString().toLowerCase());
//           if (errorMessage.contains('post not found') ||
//               errorMessage.contains('bài viết không tồn tại')) {
//             isPostDeleted = true;
//           }
...
//                 GestureDetector(
//                   onLongPress:
//                       widget.isRecording ? null : widget.onStartRecording,
...
//                       ? () {
//                           print(
//                               '🔵 [DirectMessageInput] Stop recording button pressed');
//                           widget.onStopRecording(send: true);
//                         }
...
//                           : () {
//                               print(
//                                   '🔵 [DirectMessageInput] Send button pressed');
//                               print(
//                                   '   Controller text: "${widget.controller.text}"');
//                               print(
//                                   '   Controller text trimmed: "${widget.controller.text.trim()}"');
//                               print(
//                                   '   Is empty: ${widget.controller.text.trim().isEmpty}');
//                               print('   _hasText: $_hasText');
//                               try {
//                                 widget.onSend();
//                                 print(
//                                     '✅ [DirectMessageInput] onSend callback executed');
//                               } catch (e, stackTrace) {
//                                 print(
//                                     '❌ [DirectMessageInput] Error in onSend callback: $e');
//                                 print(
//                                     '❌ [DirectMessageInput] Stack trace: $stackTrace');
//                               }
//                             }),
...
//     final theme = Theme.of(context);
//     final isMe =
//         currentResidentId != null && message.senderId == currentResidentId;
...
//                   theme: theme,
//                 )
//               else if (message.messageType == 'MARKETPLACE_POST' &&
//                   message.content != null)
...
//   @override
//   State<_DirectAudioMessageWidget> createState() =>
//       _DirectAudioMessageWidgetState();
// }
...
//   @override
//   State<_DirectFileMessageWidget> createState() =>
//       _DirectFileMessageWidgetState();
// }
...
//       if (isSender) {
//         final localPath =
//             await MessageLocalPathService.getLocalPath(widget.messageId);
//         if (localPath != null) {
...
//           widget.mimeType, widget.fileName);
//       final existingPath = await PublicFileStorageService.getExistingFilePath(
//         widget.fileName,
//         fileType,
//         widget.mimeType,
...
//   @override
//   State<_DirectVideoMessageWidget> createState() =>
//       _DirectVideoMessageWidgetState();
// }
...
//         debugPrint(
//             '⚠️ [DirectChatVideo] Skipping ImageKit video (out of storage): $videoUrl');
//         if (mounted) {
//           setState(() {
...
//       final fileType =
//           PublicFileStorageService.getFileType('video/mp4', widget.fileName);
//       await PublicFileStorageService.downloadAndSave(
//         widget.videoUrl,
//         widget.fileName,
//         fileType,
