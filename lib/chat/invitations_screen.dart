...
class _InvitationsScreenState extends State<InvitationsScreen> {
  ...
  Future<void> _loadInvitations() async {
    ...
    print(
        '📋 [InvitationsScreen] ========== _loadInvitations START ==========');
    ...
    try {
      ...
      print(
          '✅ [InvitationsScreen] Loaded ${groupInvitations.length} group invitations');
    } catch (e, stackTrace) {
    ...
    try {
      ...
      print(
          '✅ [InvitationsScreen] Loaded ${directInvitations.length} direct invitations');

      // Filter only PENDING invitations
      directInvitations =
          directInvitations.where((inv) => inv.status == 'PENDING').toList();
      print(
          '✅ [InvitationsScreen] Filtered to ${directInvitations.length} PENDING direct invitations');
    } catch (e, stackTrace) {
    ...
    print(
        '📋 [InvitationsScreen] Total invitations loaded: $totalCount (${groupInvitations.length} group + ${directInvitations.length} direct)');
    ...
    if (errors.isNotEmpty && totalCount == 0) {
      ...
      print(
          '⚠️ [InvitationsScreen] Some errors occurred but ${totalCount} invitations loaded');
    }
  ...
  Future<void> _acceptGroupInvitation(
      GroupInvitationResponse invitation) async {
    try {
  ...
  Future<void> _declineGroupInvitation(
      GroupInvitationResponse invitation) async {
    try {
  ...
  Future<void> _acceptDirectInvitation(DirectInvitation invitation) async {
    try {
      print(
          '📤 [InvitationsScreen] Accepting direct invitation: ${invitation.id}');

      final acceptedInvitation =
          await _service.acceptDirectInvitation(invitation.id);
  ...
  Widget build(BuildContext context) {
    ...
    print(
        '📋 [InvitationsScreen]   _groupInvitations.length: ${_groupInvitations.length}');
    print(
        '📋 [InvitationsScreen]   _directInvitations.length: ${_directInvitations.length}');
                          ...
                          Expanded(
                            child: ListView.builder(
                              ...
                              itemCount: totalCount +
                                  (_groupInvitations.isNotEmpty
                                      ? 1
                                      : 0) + // Group header
                                  (_directInvitations.isNotEmpty
                                      ? 1
                                      : 0), // Direct header
                              itemBuilder: (context, index) {
                                ...
                                if (currentIndex < _groupInvitations.length) {
                                  final invitation =
                                      _groupInvitations[currentIndex];
                                ...
                                final invitation =
                                    _directInvitations[directIndex];
                                return Card(
