...
class _DirectFilesScreenState extends State<DirectFilesScreen> with SingleTickerProviderStateMixin , SafeStateMixin<DirectFilesScreen> {
  ...
  void _onScroll() {
    // Removed auto-load on scroll - only load when user clicks "Hiển thị thêm" button
    // This prevents lag when displaying too many files
  }
  ...
  Future<void> _downloadImage(DirectChatFile file) async {
    try {
      ...
      final existingPath = await PublicFileStorageService.getExistingFilePath(file.fileName, fileType, mimeType);
                ...
                child: Center(
                  child: _isLoadingMore
                      ...
                      : OutlinedButton.icon(
                          ...
                          label: const Text('Hiển thị thêm ảnh'),
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildBuilderDelegate(
                ...
                childCount: _documentFiles.length,
              ),
            ),
          ),
          if (_hasMore)
                ...
                child: Center(
                  child: _isLoadingMore
                      ...
                      : OutlinedButton.icon(
                          onPressed: _loadMoreFiles,
                          icon: const Icon(CupertinoIcons.arrow_down, size: 16),
                          label: const Text('Hiển thị thêm file'),
                          style: OutlinedButton.styleFrom(
                            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                          ),
                        ),
